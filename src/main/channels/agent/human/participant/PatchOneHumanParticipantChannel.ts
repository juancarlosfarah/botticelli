import { PATCH_ONE_HUMAN_PARTICIPANT_CHANNEL } from '@shared/channels';
...
export class PatchOneHumanParticipantChannel extends PatchOneChannel {
  constructor() {
    super({
      name: PATCH_ONE_HUMAN_PARTICIPANT_CHANNEL,
  ...
  async handle(
    ...
    // update the agent
    await AppDataSource.manager.update(HumanParticipant, id, {
      description,
      name,
    });
